			...
			<div class="card-header">
				...
				<div class="card-header-action">
					<div class="input-group-btn" id="">
						<a name="" id="btn-tambah-kasir" class="btn btn-primary" href="#form-tambah-kasir" role="button"><i class="fas fa-plus"></i> Tambah Kasir</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<div class="table-reponsive">
					<table class="table table-sm table-striped" id="pengguna-kasir-table" style="width: 100%;">
						<thead>
	...
	<div class="modal-dialog modal-lg modal-dialog-scrollable" style="max-width: 1000px !important;">
		<div class="modal-content">
			<div class="modal-header px-4">
				<h5 class="modal-title" id="exampleModalLongTitle">Detail Kasir</h5>
...
// 		grade()
	...
	function refreshTable() {
		var table = $("#pengguna-kasir-table").DataTable();
		table.ajax.reload();
	}
	...
	function dataKasir() {
		$.ajax({
			type: 'get',
			url: `<?= base_url('pengguna-kasir-data') ?>`,
			dataType: 'json',
			success: function(response) {
				$('#pengguna-kasir-table').DataTable({
					processing: true,
					data: response,
			...
			$.ajax({
				...
				success: function(response) {
					if (response.res == 'nama') {
						...
						Swal.fire({
							title: 'Apakah anda yakin ingin menambahkan data kasir ?',
							icon: 'question',
							...
							if (result.isConfirmed) {
								...
								$.ajax({
									type: 'post',
									url: `<?= base_url('tambah-data-kasir') ?>`,
									data: fd,
									processData: false,
									contentType: false,
									dataType: 'json',
...
// 		$('#gradePegawai2').removeAttr('disabled')
				...
				$.ajax({
					...
					success: function(response) {
						if (response.res == 'success') {
							...
							$('#status-akun-aktif').hide()
							detailPegawai(id)
							$('#pengguna-kasir-table').DataTable().destroy()
							dataKasir()
						} else {
							Swal.fire({
								icon: 'error',
	...
	$('.btn-perbarui-pegawai').click(function() {
		...
		$.ajax({
			type: 'post',
			url: `<?= base_url('cek-data-kasir-detail') ?>`,
			data: {
				id: id,
				nama: nama,
				username: username,
