...
<div class="modal fade bd-example-modal-lg" id="modal-profil-pegawai" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
...
<form action="<?= base_url('cetak-kartu-anggota') ?>" target="_blank" method="POST" id="cetakKartuAnggota" hidden>
	<input type="text" value="<?= $this->session->userdata('id') ?>" name="id">
</form>
...
<script>
	...
	function profilPegawai(id) {
		disabled()
		$.ajax({
			type: 'POST',
			url: `<?= base_url('detail-pegawai2') ?>`,
			data: {
				id: id
			},
			dataType: "JSON",
		...
		$('#modal-profil-pegawai').appendTo("body")
		$('#modal-profil-pegawai').modal('show')
	}
	...
	$('#btn-ubah-ktp').click(function() {
		...
		$('#modal-profil-pegawai').modal('hide')
	...
	$('.btn-perbarui-pegawai').click(function() {
		...
		$.ajax({
			type: 'post',
			url: `<?= base_url('cek-data-pegawai-detail2') ?>`,
			data: {
				id: id,
				nama: nama,
				username: username,
				...
				if (response.res == 'nama') {
					...
					Swal.fire({
						...
						if (result.isConfirmed) {
							...
							$.ajax({
								type: 'post',
								url: `<?= base_url('ubah-data-pegawai2') ?>`,
								data: fd,
								processData: false,
								contentType: false,
								dataType: 'json',
